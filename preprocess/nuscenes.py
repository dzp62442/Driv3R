...
class NuSceneDataset():
    ...
    def get_metas(self):
        ...
        # sensor maps, currently only CAM_FRONT
        sensor_maps = {}
        with open(os.path.join(self.data_root, "v1.0-trainval", "sensor.json"), 'r') as f:
            sensors = json.load(f)
        
        for sensor in tqdm.tqdm(sensors, desc="loading sensor"):
            sensor_maps[sensor["token"]] = sensor["channel"]
        
        # calibarated sensor
        calibrated_sensor_maps = {}
        with open(os.path.join(self.data_root, "v1.0-trainval", "calibrated_sensor.json"), 'r') as f:
            calibrated_sensors = json.load(f)
        
        for calibrated_sensor in tqdm.tqdm(calibrated_sensors, desc="loading calibrated sensor"):
            calibrated_sensor_maps[calibrated_sensor["token"]] = dict(
                channel=sensor_maps[calibrated_sensor["sensor_token"]],
        ...
        # ego pose
        ego_pose_maps = {}
        with open(os.path.join(self.data_root, "v1.0-trainval", "ego_pose.json"), 'r') as f:
            ego_poses = json.load(f)
        
        for ego_pose in tqdm.tqdm(ego_poses, desc="loading ego pose"):
            ego_pose_maps[ego_pose["token"]] = dict(
        ...
        # sample
        sample_maps = {}
        with open(os.path.join(self.data_root, "v1.0-trainval", "sample.json"), 'r') as f:
            samples = json.load(f)
        
        for sample in tqdm.tqdm(samples, desc="loading samples"):
            sample_maps[sample["token"]] = sample["scene_token"]

        # scene
        scene_token_list = []
        scene_dict = {}
        with open(os.path.join(self.data_root, "v1.0-trainval", "scene.json"), 'r') as f:
            scenes = json.load(f)

        for scene in tqdm.tqdm(scenes, desc="loading scenes"):
            scene_token_list.append(scene["token"])
            scene_dict[scene["name"]] = scene["token"]
        
        # sample data
        sample_data_maps = {}
        for scene_token in scene_token_list:
            sample_data_maps[scene_token] = {}

        with open(os.path.join(self.data_root, "v1.0-trainval", "sample_data.json"), 'r') as f:
            sample_datas = json.load(f)
        
        for sample_data in tqdm.tqdm(sample_datas, desc="loading sample datas"):
            if sample_data["fileformat"] == "jpg" and sample_data["is_key_frame"]:
                sample_token = sample_data["sample_token"]
                scene_token = sample_maps[sample_token]
              
                if sample_token not in sample_data_maps[scene_token].keys():
                    sample_data_maps[scene_token][sample_token] = {}
                
                calibrated_sensor = calibrated_sensor_maps[sample_data["calibrated_sensor_token"]]
                ego_pose = ego_pose_maps[sample_data["ego_pose_token"]]
                timestamp = sample_data["timestamp"]
                filename = sample_data["filename"]
                height = sample_data["height"]
                width = sample_data["width"]

                sensor_channel = calibrated_sensor["channel"]
                sample_data_maps[scene_token][sample_token][sensor_channel] = dict(
                    timestamp=timestamp,
                    filename=filename,
                    ego_pose=ego_pose,
        ...
        for sample_data in tqdm.tqdm(sample_datas, desc="loading lidar pcd"):
            sample_token = sample_data["sample_token"]
            scene_token = sample_maps[sample_token]

            if "LIDAR_TOP" in sample_data["filename"] and sample_data["is_key_frame"]:

                assert sample_token in sample_data_maps[scene_token]
        ...
        train_tokens = [scene_dict[scene] for scene in train]
        val_tokens = [scene_dict[scene] for scene in val]
        ...
        with open(os.path.join(self.data_root, 'train_metas.pkl'), 'wb') as f:
            pickle.dump(train_metas, f)
        print(f"Training set {len(train_metas)} scenes.")
        with open(os.path.join(self.data_root, 'val_metas.pkl'), 'wb') as f:
            pickle.dump(val_metas, f)
        print(f"Validate set {len(val_metas)} scenes.")
